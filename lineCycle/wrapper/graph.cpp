...
PointF * intersection(float ax, float ay, float bx, float by, float cx, float cy, float dx, float dy,CMemPool * theMemPool)
{
	float a1 = by - ay;
	float b1 = ax - bx;
	...
	PointF * res = (PointF *)theMemPool->AllocNTS(sizeof(PointF));
	res->x = x, res->y = y;
	return res;
}
